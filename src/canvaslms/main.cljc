(ns canvaslms.main
  "Kotodama WASM entrypoint for the Canvaslms clean-room actor (L4) — Clojure port.
  ...
  facts (`canvaslms.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "canvaslms")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"canvaslms.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "canvaslms-compat" :tier tier :entities entities} 200])
...
(defn start! [] :canvaslms-compat/ready)
